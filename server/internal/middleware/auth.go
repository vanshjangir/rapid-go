...
func verifyGoogleToken(ctx context.Context, token string) (
	*oauth2.Tokeninfo, error,
) {
	oauth2Service, err := oauth2.NewService(
		ctx, option.WithAPIKey(os.Getenv("GOOGLE_CLIENT_ID")),
	)
	if err != nil {
		return nil, err
	}
...
func auth(ctx *gin.Context, token string) {
	if token == "" {
		ctx.JSON(
			...
			gin.H{"error": "Token query param missing"},
		)
		ctx.Abort()
		return
	}
	...
	case TOKEN_TYPE_OAUTH:
		...
		if err != nil {
			...
			ctx.JSON(
				http.StatusUnauthorized,
				gin.H{"error": "Invalid Google token"},
			)
			ctx.Abort()
			return
		}
		...
		if err := db.QueryRow(query, tokenInfo.Email).Scan(&username); err != nil {
			log.Println("Google query failed")
			ctx.JSON(
				http.StatusUnauthorized,
				gin.H{"error": "Username not found"},
			)
			ctx.Abort()
			return
		}
	...
	default:
		...
		ctx.JSON(
			http.StatusUnauthorized,
			gin.H{"error": "Unsupported token type"},
		)
		ctx.Abort()
	}
}
